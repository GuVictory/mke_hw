...
pair<CsrMatrix, Vec> GlobalEqGenSystem(const Triangulation& t,
                                         LocalEqGen gen)
{
...
pair<DenseMatrix, Vec> GlobalEqGenDense(const Triangulation& t,
                                                 LocalEqGen gen)
{
    auto m = Index(t.nodes().size());
    ...
    //! Работа с граничными условиями 1-го рода
    for (auto& fc : t.first()) {
        for (auto& n : fc) {
            auto gn = n.index();
            auto cn = t.on_first(n);
